...
{
  static const double b_a[9]{4.8084E+6, 0.0, -251075.99999999997, 0.0,
                             7.68E+6,   0.0, -251075.99999999997, 0.0,
                             1.19904E+7};
  static const double c_a[9]{-11.0, 0.0, 1.7, 0.0,  -14.006420569329665,
                             0.0,   5.0, 0.0, -11.5};
  ...
  double C_bs_tmp;
  double H_phi_tmp;
  double MEcg1_b_idx_1;
  double MEcg1_b_idx_2;
  double Va;
  double a;
  double alpha;
  double b_C_bs_tmp;
  ...
  MEcg1_b_idx_1 = 2.56 * FE1_b[0];
  MEcg1_b_idx_2 = 0.0 - -7.94 * FE1_b[0];
  // GRAVITY
  g_b_tmp = std::sin(X[6]);
  b_g_b_tmp = std::cos(X[6]);
  c_g_b_tmp = std::cos(X[7]);
  // STATE
  // DERIVATIVES-----------------------------------------------------------
  H_phi_tmp = std::tan(X[7]);
  b_FE1_b[0] = FE1_b[0] + u5 * 120000.0 * 9.81;
  b_FE1_b[1] = 0.0;
  b_FE1_b[2] = 0.0;
  ...
  FE1_b[0] = -1.4 * beta;
  FE1_b[1] = -0.59 - 124.90043076923077 * (alpha - -0.20071286397934787);
  FE1_b[2] = ((1.0 - alpha) + 3.8197186342054885) * beta;
  b_C_bs_tmp = -(0.07 * (a * a) + 0.13) * 260.0 * qdach;
  C_bs_tmp = (-1.6 * beta + 0.24 * u3) * 260.0 * qdach;
  if (alpha <= 0.2530727415391778) {
    u4 = 5.5 * (alpha - -0.20071286397934787);
  } else {
    u4 = ((-768.5 * rt_powd_snf(alpha, 3.0) + 609.2 * (alpha * alpha)) +
          -155.2 * alpha) +
         15.212;
  }
  u4 = -(u4 + 0.7630769230769231 *
                  (((alpha - 0.25 * (alpha - -0.20071286397934787)) + u2) +
                   0.3 * X[4] * 24.8 / Va)) *
       260.0 * qdach;
  for (int i{0}; i < 3; i++) {
    y[i] = (b_a[i] * wbe_b_idx_0 + b_a[i + 3] * wbe_b_idx_1) +
           b_a[i + 6] * wbe_b_idx_2;
    FA_b[i] = (c_C_bs_tmp[i] * b_C_bs_tmp + c_C_bs_tmp[i + 3] * C_bs_tmp) +
              c_C_bs_tmp[i + 6] * u4;
    dv[i] = (FE1_b[i] + ((c_a[i] * wbe_b_idx_0 + c_a[i + 3] * wbe_b_idx_1) +
                         c_a[i + 6] * wbe_b_idx_2)) +
            ((d_a[i] * u1 + d_a[i + 3] * u2) + d_a[i + 6] * u3);
  }
  FE1_b[0] = 120000.0 * (-9.81 * std::sin(X[7])) + FA_b[0];
  FE1_b[1] = 120000.0 * (9.81 * c_g_b_tmp * g_b_tmp) + FA_b[1];
  FE1_b[2] = 120000.0 * (9.81 * std::cos(X[7]) * b_g_b_tmp) + FA_b[2];
  u4 = (dv[0] * qdach * 260.0 * 6.6 + (FA_b[1] * 0.66 - FA_b[2] * 0.0)) -
       (y[2] * X[4] - y[1] * X[5]);
  MEcg1_b_idx_1 = ((MEcg1_b_idx_1 + MEcg1_b_idx_1) +
                   (dv[1] * qdach * 260.0 * 6.6 +
                    (FA_b[2] * -77.681999999999988 - FA_b[0] * 0.66))) -
                  (y[0] * X[5] - y[2] * X[3]);
  MEcg1_b_idx_2 = ((MEcg1_b_idx_2 + MEcg1_b_idx_2) +
                   (dv[2] * qdach * 260.0 * 6.6 +
                    (FA_b[0] * 0.0 - FA_b[1] * -77.681999999999988))) -
                  (y[1] * X[3] - y[0] * X[4]);
  c_C_bs_tmp[0] = 1.0;
  c_C_bs_tmp[3] = g_b_tmp * H_phi_tmp;
  c_C_bs_tmp[6] = b_g_b_tmp * H_phi_tmp;
  c_C_bs_tmp[1] = 0.0;
  c_C_bs_tmp[4] = b_g_b_tmp;
  c_C_bs_tmp[7] = -g_b_tmp;
  c_C_bs_tmp[2] = 0.0;
  c_C_bs_tmp[5] = g_b_tmp / c_g_b_tmp;
  c_C_bs_tmp[8] = b_g_b_tmp / c_g_b_tmp;
  for (int i{0}; i < 3; i++) {
    Xdot[i] = 8.3333333333333337E-6 * (FE1_b[i] + b_FE1_b[i]) - wbe_b[i];
    Xdot[i + 3] =
        (e_a[i] * u4 + e_a[i + 3] * MEcg1_b_idx_1) + e_a[i + 6] * MEcg1_b_idx_2;
    Xdot[i + 6] =
